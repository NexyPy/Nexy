...
from nexy.compiler.parser.nodes import Node, TextNode, ElementNode
    ...
    def format_dict(cls, attrs: Dict[str, str]) -> str:
        ...
        for key, value in attrs.items():
            ...
            if "{{" in value:
                ...
                if value.strip().startswith("{{") and value.strip().endswith("}}") and value.count("{{") == 1:
...
class TemplateParser:
            ...
            if isinstance(node, ElementNode):
                if node.is_component():
                    ...
                    if tag_name not in self.known_components and tag_name != "Slot":
                        raise NexyParserError(f"Missing Import: <{tag_name}> used but not declared.")
    ...
    def _render_component(self, node: ElementNode) -> str:
        ...
        unprotected_attrs = {self._unprotect_jinja(k): self._unprotect_jinja(v) for k, v in node.attributes.items()}
