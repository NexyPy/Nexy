...
class NexyVFSFinder(importlib.abc.MetaPathFinder):
    ...
    def find_spec(self, fullname: str, path: Optional[list[str]], target: Optional[object] = None) -> Optional[importlib.machinery.ModuleSpec]:
        ...
        if self.vfs.exists(init_path):
            return importlib.util.spec_from_loader(fullname, NexyVFSLoader(init_path), is_package=True)
