...
export function checkFrameworks(usedFrameworks: Set<Framework>): Set<Framework> {
  ...
  for (const framework of usedFrameworks) {
    ...
    if (fs.existsSync(pkgPath)) {
      ...
      console.warn(
        `${c.yellow}[nexy] "${framework}" is used but not installed.` +
...
export function getManifest(): Record<string, any> {
  ...
  if (!fs.existsSync(manifestPath)) {
    console.error(`${c.red}[nexy] Manifest not found. Run vite build first.${c.reset}`)
    process.exit(1)
  }
...
export function writeComponent(
  ...
  console.log(`${c.dim}[nexy] __nexy__/client/static/${snippetKey}${c.reset}`)
}
